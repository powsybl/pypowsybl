...
def test_remove_extensions():
    ...
    assert n.get_extension('activePowerControl').empty
...
def test_merged_xnode():
    ...
    network.update_extensions('mergedXnode', id='BBBBBB11 XXXXXX11 1 + FFFFFF11 XXXXXX11 1', code='XXXXXX15',
                              ...
                              p1=0, q1=0, p2=0, q2=0)
    x = network.get_extension('mergedXnode').loc['BBBBBB11 XXXXXX11 1 + FFFFFF11 XXXXXX11 1']
    ...
    assert network.get_extension('mergedXnode').empty
...
def test_xnode():
    ...
    e = network.get_extension('xnode').loc['NNL2AA1  XXXXXX11 1']
    assert e.code == 'XXXXXX12'

    network.remove_extensions('xnode', ['NNL2AA1  XXXXXX11 1'])
    assert network.get_extension('xnode').empty
...
def test_entsoe_area():
    ...
    e = network.get_extension('entsoeArea').loc['D4NEUR']
    assert e.code == 'FR'

    network.remove_extensions('entsoeArea', ['D4NEUR'])
    assert network.get_extension('entsoeArea').empty
...
def test_entsoe_category():
    ...
    e = network.get_extension('entsoeCategory').loc['GEN']
    assert e.code == 6

    network.remove_extensions('entsoeCategory', ['GEN'])
    assert network.get_extension('entsoeCategory').empty
...
def test_hvdc_angle_droop_active_power_control():
    ...
    extensions = n.get_extension(extension_name)
    assert extensions.empty

    n.create_extensions(extension_name, id=element_id, droop=0.1, p0=200, enabled=True)
    e = n.get_extension(extension_name).loc[element_id]
    ...
    assert n.get_extension(extension_name).empty
