...
def test_update_network_from_binary_buffers():
    network = pp.network.load(DATA_DIR.joinpath("load_EQ.xml"))
    assert math.isnan(network.get_loads()["p0"]["EnergyConsumer"])
    buffer = io.BytesIO()
    with zipfile.ZipFile(buffer, "w") as zf:
        zf.writestr("load_SSH.xml", DATA_DIR.joinpath("load_SSH.xml").read_bytes())
    buffer.seek(0)
    network.update_from_binary_buffer(buffer)
    assert 10.0 == network.get_loads()["p0"]["EnergyConsumer"]
...
def test_lcc_data_frame():
    ...
    expected = pd.DataFrame(index=pd.Series(name='id', data=['LCC1', 'LCC2']),
                            columns=['name', 'power_factor', 'loss_factor', 'p', 'q', 'i', 'voltage_level_id',
                                     'bus_id', 'connected', 'hvdc_line_id'],
                            data=[['LCC1', 0.6, 1.1, 80.88, nan, nan, 'S1VL2', 'S1VL2_0', True, 'HVDC2'],
    ...
    expected = pd.DataFrame(index=pd.Series(name='id',
                            ...
                            columns=['name', 'power_factor', 'loss_factor', 'p', 'q', 'i', 'voltage_level_id',
                                     'bus_id', 'connected', 'hvdc_line_id'],
                            data=[['LCC1', 0.7, 1.2, 82, 69, 154.68, 'S1VL2', 'S1VL2_0', True, 'HVDC2'],
                                  ['LCC2', 0.6, 1.1, - 79.12, nan, nan, 'S3VL1', 'S3VL1_0', True, 'HVDC2']])
