...
BBE1AA1               0 2 400.00 3000.00 0.00000 -1500.0 0.00000 0.00000 -9000.0 9000.00 -9000.0                               F
    ...
    def test_generators_data_frame(self):
        ...
        create_df = pd.DataFrame(data=[['VLGEN', 1.0, 2.0, 1.1, 2.1, 3.1, 4.1, 0, 'NGEN', 'NGEN']],
                                 columns=['voltage_level_id', 'min_p', 'max_p', 'target_p', 'target_q', 'target_v', 'rated_s', 'voltage_regulator_on', 'connectable_bus_id', 'bus_id'], index=['GEN3'])
        n.create_generators(create_df)
        df2 = n.get_generators()
        self.assertEqual(len(generators) + 1, len(df2))
        self.assertEqual(1.0, df2['min_p']['GEN3'])
        self.assertEqual(2.0, df2['max_p']['GEN3'])
