...
def test_regulated_terminal_node_breaker():
    ...
    with pytest.raises(pp.Exception):
...
def test_update_non_modifiable_data():
    ...
    with pytest.raises(pp.Exception) as context:
        n.update_generators(update)
...
def test_exception():
    n = pp.network.create_ieee14()
    with pytest.raises(pp.Exception) as e:
        n.open_switch("aa")
...
def test_validation_level():
    ...
    with pytest.raises(pp.Exception) as exc:
...
def test_ratio_tap_changer_regulated_side():
    ...
    with pytest.raises(pp.Exception, match='a regulation terminal has to be set'):
        n.update_ratio_tap_changers(id='T4-1-0', target_v=100, regulating=True)

    with pytest.raises(pp.Exception, match='must be ONE or TWO'):
...
def test_phase_tap_changer_regulated_side():
    ...
    n.update_ratio_tap_changers(id='TWT', regulating=False)
    with pytest.raises(pp.Exception, match='regulated terminal is not set'):
