...
@pytest.mark.parametrize('network_factory, bus_id, target_voltage_id', [
    # bus/breaker network: function id given as a bus view (calculated) bus id
    (pp.network.create_eurostag_tutorial_example1_network, 'VLGEN_0', 'GEN'),
    # bus/breaker network: function id given as a bus/breaker view (configured) bus id
    (pp.network.create_eurostag_tutorial_example1_network, 'NGEN', 'GEN'),
    # node/breaker network: function id given as a bus view (calculated) bus id
    (pp.network.create_four_substations_node_breaker_network, 'S3VL1_0', 'GTH2'),
    # node/breaker network: function id given as a busbar section id
...
def test_bus_voltage_sensi_id_resolution(network_factory, bus_id, target_voltage_id):
    # A BUS_VOLTAGE factor function id can be provided as any kind of bus reference: a bus view id,
    # a bus/breaker view (configured) bus id, or a busbar section id. All must be resolved to the
    # corresponding bus (SensitivityFactor.resolveBusId) so the analysis runs without raising
    # "bus not found". In each case the target voltage generator regulates the referenced bus, so
    # the self voltage sensitivity is 1.0.
