...
from typing import Any, ClassVar, Dict, Iterator, List, Optional, Sequence, Union
...
class PostContingencyComputationStatus:
    __members__: ClassVar[Dict[str, PostContingencyComputationStatus]] = (
        ...
    )  # read-only
...
class SeriesMetadata:
    def __init__(
        self, arg0: str, arg1: int, arg2: bool, arg3: bool, arg4: bool
    ) -> None: ...
    @property
...
class Zone:
    def __init__(
        self, id: str, injections_ids: List[str], injections_shift_keys: List[float]
    ) -> None: ...
...
class VoltageInitializerStatus:
    __members__: ClassVar[Dict[str, VoltageInitializerStatus]] = ...  # read-only
    OK: ClassVar[VoltageInitializerStatus] = ...
    NOT_OK: ClassVar[VoltageInitializerStatus] = ...

class VoltageInitializerObjective:
    __members__: ClassVar[Dict[str, VoltageInitializerObjective]] = ...  # read-only
...
class VoltageInitializerLogLevelAmpl:
    __members__: ClassVar[Dict[str, VoltageInitializerLogLevelAmpl]] = ...  # read-only
...
class VoltageInitializerLogLevelSolver:
    __members__: ClassVar[Dict[str, VoltageInitializerLogLevelSolver]] = (
        ...
    )  # read-only
...
class VoltageInitializerReactiveSlackBusesMode:
    __members__: ClassVar[Dict[str, VoltageInitializerReactiveSlackBusesMode]] = (
        ...
    )  # read-only
...
def add_contingency(
    analysis_context: JavaHandle, contingency_id: str, elements_ids: List[str]
) -> None: ...
def add_contingency_from_json_file(
    analysis_context: JavaHandle, path_to_json_file: str
) -> None: ...
def export_to_json(result: JavaHandle, path: str) -> None: ...
def add_monitored_elements(
    security_analysis_context: JavaHandle,
    contingency_context_type: ContingencyContextType,
    branch_ids: List[str],
    voltage_level_ids: List[str],
    three_windings_transformer_ids: List[str],
...
def add_switch_action(
    security_analysis_context: JavaHandle, action_id: str, switch_id: str, open: bool
) -> None: ...
...
def add_shunt_compensator_position_action(
    security_analysis_context: JavaHandle, action_id: str, shunt_id: str, section: int
) -> None: ...
...
def add_operator_strategy(
    ...
    violation_subject_ids: List[str],
    violation_types: List[ViolationType],
) -> None: ...
def add_action_from_json_file(
    security_analysis_context: JavaHandle, path_to_json_file: str
) -> None: ...
def add_operator_strategy_from_json_file(
    security_analysis_context: JavaHandle, path_to_json_file: str
) -> None: ...
def clone_variant(
    network: JavaHandle, src: str, variant: str, may_overwrite: bool
) -> None: ...
...
def create_element(
    network: JavaHandle,
    dataframes: List[Optional[Dataframe]],
...
def create_network(
    name: str, id: str, allow_variant_multi_thread_access: bool
) -> JavaHandle: ...
def create_network_elements_series_array(
    network: JavaHandle,
    element_type: ElementType,
    filter_attributes_type: FilterAttributesType,
    attributes: List[str],
    array: Optional[Dataframe],
    per_unit: bool,
    nominal_apparent_power: float,
) -> SeriesArray: ...
def create_network_elements_extension_series_array(
    network: JavaHandle, extension_name: str, table_name: str
) -> SeriesArray: ...
...
def save_network(
    ...
    report: Optional[JavaHandle],
) -> None: ...
def save_network_to_string(
    network: JavaHandle,
    format: str,
    parameters: Dict[str, str],
    report: Optional[JavaHandle],
) -> str: ...
def save_network_to_binary_buffer(
    network: JavaHandle,
    format: str,
    parameters: Dict[str, str],
    report: Optional[JavaHandle],
...
def get_sensitivity_matrix(
    sensitivity_analysis_result_context: JavaHandle, matrix_id: str, contingency_id: str
) -> Matrix: ...
def get_branch_results(result: JavaHandle) -> SeriesArray: ...
def get_bus_breaker_view_buses(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
def get_bus_breaker_view_elements(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
def get_bus_breaker_view_switches(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
def get_bus_results(result: JavaHandle) -> SeriesArray: ...
def get_loadflow_provider_parameters_names(provider: str) -> List[str]: ...
def create_loadflow_parameters_from_json(
    parameters_json: str,
) -> LoadFlowParameters: ...
...
def get_network_area_diagram_svg(
    network: JavaHandle,
...
def get_network_area_diagram_svg_and_metadata(
    ...
    low_nominal_voltage_bound: float,
    nad_parameters: NadParameters,
    fixed_positions: Optional[Dataframe],
    branch_labels: Optional[Dataframe],
    three_wt_labels: Optional[Dataframe],
    injections_labels: Optional[Dataframe],
    bus_descriptions: Optional[Dataframe],
    vl_descriptions: Optional[Dataframe],
    bus_node_styles: Optional[Dataframe],
    edge_styles: Optional[Dataframe],
    three_wt_styles: Optional[Dataframe],
) -> List[str]: ...
def get_network_area_diagram_displayed_voltage_levels(
    network: JavaHandle, voltage_level_ids: Union[str, List[str]], depth: int
) -> List[str]: ...
def get_network_elements_ids(
    network: JavaHandle,
    element_type: ElementType,
    nominal_voltages: List[float],
    countries: List[str],
    main_connected_component: bool,
    main_synchronous_component: bool,
    not_connected_to_same_bus_at_both_sides: bool,
) -> List[str]: ...
...
def get_node_breaker_view_internal_connections(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
def get_node_breaker_view_nodes(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
def get_node_breaker_view_switches(
    network: JavaHandle, voltage_level: str
) -> SeriesArray: ...
...
def get_operator_strategy_results(
    result: JavaHandle,
) -> OperatorStrategyResultArray: ...
def get_pre_contingency_result(result: JavaHandle) -> PreContingencyResult: ...
def get_network_elements_dataframe_metadata(
    element_type: ElementType,
) -> List[SeriesMetadata]: ...
def get_network_elements_creation_dataframes_metadata(
    element_type: ElementType,
) -> List[List[SeriesMetadata]]: ...
def get_single_line_diagram_svg(network: JavaHandle, container_id: str) -> str: ...
def get_single_line_diagram_svg_and_metadata(
    network: JavaHandle, container_id: str, parameters: SldParameters
) -> List[str]: ...
def get_matrix_multi_substation_single_line_diagram_svg_and_metadata(
    network: JavaHandle, matrix_ids: List[List[str]], parameters: SldParameters
) -> List[str]: ...
...
def add_factor_matrix(
    sensitivity_analysis_context: JavaHandle,
    matrix_id: str,
    ...
    sensitivity_variable_type: Optional[SensitivityVariableType],
) -> None: ...
...
def load_network_from_binary_buffers(
    file_content: List[memoryview],
    parameters: Dict[str, str],
    post_processors: List[str],
    report: Optional[JavaHandle],
    allow_variant_multi_thread_access: bool,
...
def run_sensitivity_analysis(
    ...
    network: JavaHandle,
    dc: bool,
    parameters: SensitivityAnalysisParameters,
    provider: str,
    report: Optional[JavaHandle],
) -> JavaHandle: ...
...
def set_min_validation_level(
    network: JavaHandle, validation_level: ValidationLevel
) -> None: ...
...
def update_network_elements_with_series(
    network: JavaHandle,
    array: Dataframe,
    element_type: ElementType,
    per_unit: bool,
...
def add_network_element_properties(
    network: JavaHandle, dataframe: Dataframe
) -> None: ...
def remove_network_element_properties(
    network: JavaHandle, ids: List[str], properties: List[str]
) -> None: ...
...
def update_extensions(
    network: JavaHandle, name: str, table_name: str, dataframe: Dataframe
) -> None: ...
def get_network_extensions_creation_dataframes_metadata(
    name: str,
) -> List[List[SeriesMetadata]]: ...
def create_extensions(
    network: JavaHandle, dataframes: List[Optional[Dataframe]], name: str
) -> None: ...
...
def get_glsk_injection_keys(
    network: JavaHandle, importer: JavaHandle, country: str, timestamp: int
) -> List[str]: ...
def get_glsk_factors(
    network: JavaHandle, importer: JavaHandle, country: str, timestamp: int
) -> List[float]: ...
def create_flow_decomposition() -> JavaHandle: ...
def add_contingency_for_flow_decomposition(
    flow_decomposition_context: JavaHandle, contingency_id: str, elements_ids: List[str]
) -> None: ...
def add_precontingency_monitored_elements_for_flow_decomposition(
    flow_decomposition_context: JavaHandle, branch_ids: List[str]
) -> None: ...
def add_postcontingency_monitored_elements_for_flow_decomposition(
    flow_decomposition_context: JavaHandle,
    branch_ids: List[str],
    contingency_ids: List[str],
) -> None: ...
def add_additional_xnec_provider_for_flow_decomposition(
    flow_decomposition_context: JavaHandle, default_xnec_provider: DefaultXnecProvider
) -> None: ...
def run_flow_decomposition(
    flow_decomposition_context: JavaHandle,
    network: JavaHandle,
    flow_decomposition_parameters: FlowDecompositionParameters,
    load_flow_parameter: LoadFlowParameters,
) -> SeriesArray: ...
...
def revert_connect_voltage_level_on_line(
    network: JavaHandle, line1_id: str, line2_id: str, line_id: str, line_name: str
) -> None: ...
def get_connectables_order_positions(
    network: JavaHandle, voltage_level_id: str
) -> SeriesArray: ...
def get_unused_order_positions(
    network: JavaHandle, busbar_section_id: str, before_or_after: str
) -> List[int]: ...
...
def run_dynamic_simulation(
    dynamic_model: JavaHandle,
    network: JavaHandle,
    dynamic_mapping: JavaHandle,
    event_mapping: JavaHandle,
    timeseries_mapping: JavaHandle,
    parameters: DynamicSimulationParameters,
    report_node: Optional[JavaHandle],
) -> JavaHandle: ...
def add_all_dynamic_mappings(
    dynamic_mapping_handle: JavaHandle,
    mapping_type: DynamicMappingType,
    dataframes: List[Optional[Dataframe]],
) -> None: ...
def get_dynamic_mappings_meta_data(
    mapping_type: DynamicMappingType,
) -> List[List[SeriesMetadata]]: ...
def get_supported_models(mapping_type: DynamicMappingType) -> List[str]: ...
def add_output_variables(
    output_variables_handle: JavaHandle,
    dynamic_id: str,
    variables: List[str],
    is_dynamic: bool,
    output_variable_type: OutputVariableType,
) -> None: ...
def add_all_event_mappings(
    event_mapping_handle: JavaHandle,
    mapping_type: EventMappingType,
    mapping_df: Dataframe,
) -> None: ...
def get_event_mappings_meta_data(
    mapping_type: EventMappingType,
) -> List[SeriesMetadata]: ...
def set_powsybl_config_location(
    absolute_path_to_config: str, config_file_name: str
) -> None: ...
def get_dynamic_simulation_results_status(
    result_handle: JavaHandle,
...
def remove_elements_modification(
    network: JavaHandle,
    connectable_ids: List[str],
    dataframe: Optional[Dataframe],
    remove_modification_type: RemoveModificationType,
    raise_exception: Optional[bool],
    report_node: Optional[JavaHandle],
) -> None: ...
def get_network_modification_metadata(
    network_modification_type: NetworkModificationType,
) -> List[SeriesMetadata]: ...
def get_network_modification_metadata_with_element_type(
    network_modification_type: NetworkModificationType, element_type: ElementType
) -> List[List[SeriesMetadata]]: ...
def create_network_modification(
    network: JavaHandle,
    dataframes: List[Optional[Dataframe]],
    network_modification_type: NetworkModificationType,
    raise_exception: Optional[bool],
...
def set_faults(
    short_circuit_analysis: JavaHandle, dfs: Optional[Dataframe]
) -> None: ...
def get_fault_results(
    result: JavaHandle, with_fortescue_result: bool
) -> SeriesArray: ...
def get_feeder_results(
    result: JavaHandle, with_fortescue_result: bool
) -> SeriesArray: ...
def get_short_circuit_limit_violations(result: JavaHandle) -> SeriesArray: ...
def get_short_circuit_bus_results(
    result: JavaHandle, with_fortescue_result: bool
) -> SeriesArray: ...
def get_faults_dataframes_metadata() -> List[SeriesMetadata]: ...
def run_shortcircuit_analysis(
    context: JavaHandle,
    network: JavaHandle,
    parameters: ShortCircuitAnalysisParameters,
    provider: str,
    report_node: Optional[JavaHandle],
) -> JavaHandle: ...
...
def voltage_initializer_add_variable_shunt_compensators(
    params_handle: JavaHandle, id_ptr: str
) -> None: ...
...
def voltage_initializer_add_specific_low_voltage_limits(
    params_handle: JavaHandle, voltage_level_id: str, is_relative: bool, limit: float
) -> None: ...
def voltage_initializer_add_specific_high_voltage_limits(
    params_handle: JavaHandle, voltage_level_id: str, is_relative: bool, limit: float
) -> None: ...
def voltage_initializer_set_objective(
    params_handle: JavaHandle, objective: VoltageInitializerObjective
) -> None: ...
def voltage_initializer_set_objective_distance(
    params_handle: JavaHandle, dist: float
) -> None: ...
def voltage_initializer_set_log_level_ampl(
    params_handle: JavaHandle, log_level_ampl: VoltageInitializerLogLevelAmpl
) -> None: ...
def voltage_initializer_set_log_level_solver(
    params_handle: JavaHandle, log_level_solver: VoltageInitializerLogLevelSolver
) -> None: ...
def voltage_initializer_set_reactive_slack_buses_mode(
    params_handle: JavaHandle,
    reactive_slack_buses_mode: VoltageInitializerReactiveSlackBusesMode,
) -> None: ...
def voltage_initializer_set_min_plausible_low_voltage_limit(
    params_handle: JavaHandle, min_plausible_low_voltage_limit: float
) -> None: ...
def voltage_initializer_set_max_plausible_high_voltage_limit(
    params_handle: JavaHandle, max_plausible_high_voltage_limit: float
) -> None: ...
def voltage_initializer_set_active_power_variation_rate(
    params_handle: JavaHandle, active_power_variation_rate: float
) -> None: ...
def voltage_initializer_set_min_plausible_active_power_threshold(
    params_handle: JavaHandle, min_plausible_active_power_threshold: float
) -> None: ...
def voltage_initializer_set_low_impedance_threshold(
    params_handle: JavaHandle, low_impedance_threshold: float
) -> None: ...
def voltage_initializer_set_min_nominal_voltage_ignored_bus(
    params_handle: JavaHandle, min_nominal_voltage_ignored_bus: float
) -> None: ...
def voltage_initializer_set_min_nominal_voltage_ignored_voltage_bounds(
    params_handle: JavaHandle, min_nominal_voltage_ignored_voltage_bounds: float
) -> None: ...
def voltage_initializer_set_max_plausible_power_limit(
    params_handle: JavaHandle, max_plausible_power_limit: float
) -> None: ...
def voltage_initializer_set_high_active_power_default_limit(
    params_handle: JavaHandle, high_active_power_default_limit: float
) -> None: ...
def voltage_initializer_set_low_active_power_default_limit(
    params_handle: JavaHandle, low_active_power_default_limit: float
) -> None: ...
def voltage_initializer_set_default_minimal_qp_range(
    params_handle: JavaHandle, default_minimal_qp_range: float
) -> None: ...
def voltage_initializer_set_default_qmax_pmax_ratio(
    params_handle: JavaHandle, default_qmax_pmax_ratio: float
) -> None: ...
def voltage_initializer_set_default_variable_scaling_factor(
    params_handle: JavaHandle, default_variable_scaling_factor: float
) -> None: ...
def voltage_initializer_set_default_constraint_scaling_factor(
    params_handle: JavaHandle, default_constraint_scaling_factor: float
) -> None: ...
def voltage_initializer_set_reactive_slack_variable_scaling_factor(
    params_handle: JavaHandle, reactive_slack_variable_scaling_factor: float
) -> None: ...
def voltage_initializer_set_twt_ratio_variable_scaling_factor(
    params_handle: JavaHandle, twt_ratio_variable_scaling_factor: float
) -> None: ...
def run_voltage_initializer(
    debug: bool, network_handle: JavaHandle, params_handle: JavaHandle
) -> JavaHandle: ...
def voltage_initializer_apply_all_modifications(
    result_handle: JavaHandle, network_handle: JavaHandle
) -> None: ...
def voltage_initializer_get_status(
    result_handle: JavaHandle,
) -> VoltageInitializerStatus: ...
def voltage_initializer_get_indicators(result_handle: JavaHandle) -> Dict[str, str]: ...
def create_rao() -> JavaHandle: ...
def run_rao(
    network: JavaHandle, rao_context: JavaHandle, parameters: RaoParameters
) -> JavaHandle: ...
def run_voltage_monitoring(
    network: JavaHandle,
    result_handle: JavaHandle,
    context_handle: JavaHandle,
    parameters: LoadFlowParameters,
    provider: str,
) -> JavaHandle: ...
...
def set_crac_source(
    network: JavaHandle, rao_context: JavaHandle, crac_source: memoryview
) -> None: ...
def set_glsk_source(
    network: JavaHandle, rao_context: JavaHandle, glsk_source: memoryview
) -> None: ...
def get_crac(context_handle: JavaHandle) -> JavaHandle: ...
def serialize_rao_results_to_buffer(
    rao_handle: JavaHandle, crac_handle: JavaHandle
) -> bytes: ...
def get_rao_result_status(rao_handle: JavaHandle) -> RaoComputationStatus: ...
def get_flow_cnec_results(
    crac_handle: JavaHandle, result_handle: JavaHandle
) -> SeriesArray: ...
def get_angle_cnec_results(
    crac_handle: JavaHandle, result_handle: JavaHandle
) -> SeriesArray: ...
def get_voltage_cnec_results(
    crac_handle: JavaHandle, result_handle: JavaHandle
) -> SeriesArray: ...
def get_ra_results(
    crac_handle: JavaHandle, result_handle: JavaHandle
) -> SeriesArray: ...
def get_cost_results(
    crac_handle: JavaHandle, result_handle: JavaHandle
) -> SeriesArray: ...
def get_virtual_cost_names(result_handle: JavaHandle) -> List[str]: ...
def get_virtual_cost_results(
    crac_handle: JavaHandle, result_handle: JavaHandle, virtual_cost_name: str
) -> SeriesArray: ...
...
def create_grid2op_backend(
    backend: JavaHandle,
    consider_open_branch_reactive_flow: bool,
    check_isolated_and_disconnected_injections: bool,
    buses_per_voltage_level: int,
    connect_all_elements_to_first_bus: bool,
) -> JavaHandle: ...
def free_grid2op_backend(backend: JavaHandle) -> None: ...
def get_grid2op_string_value(
    backend: JavaHandle, value_type: Grid2opStringValueType
) -> List[str]: ...
def get_grid2op_integer_value(
    backend: JavaHandle, value_type: Grid2opIntegerValueType
) -> ndarray: ...
def get_grid2op_double_value(
    backend: JavaHandle, value_type: Grid2opDoubleValueType
) -> ndarray: ...
def update_grid2op_double_value(
    backend: JavaHandle,
    value_type: Grid2opUpdateDoubleValueType,
    value: ndarray,
    changed: ndarray,
) -> None: ...
def update_grid2op_integer_value(
    backend: JavaHandle,
...
def run_grid2op_loadflow(
    backend: JavaHandle, dc: bool, parameters: LoadFlowParameters
) -> LoadFlowComponentResultArray: ...
