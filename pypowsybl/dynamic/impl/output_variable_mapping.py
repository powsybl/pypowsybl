...
class OutputVariableMapping:
    ...
    def add_dynamic_model_curves(
        self, dynamic_model_id: str, variables: Union[List[str], str]
    ) -> None:
        """
        ...
        self._add_output_variables(
            dynamic_model_id, variables, True, OutputVariableType.CURVE
        )

    def add_standard_model_curves(
        self, static_id: str, variables: Union[List[str], str]
    ) -> None:
        """
        ...
        if isinstance(variables, str):
            variables = [variables]
        self._add_output_variables(
            static_id, variables, False, OutputVariableType.CURVE
        )

    def add_dynamic_model_final_state_values(
        self, dynamic_model_id: str, variables: Union[List[str], str]
    ) -> None:
        """
        ...
        if isinstance(variables, str):
            variables = [variables]
        self._add_output_variables(
            dynamic_model_id, variables, True, OutputVariableType.FINAL_STATE
        )

    def add_standard_model_final_state_values(
        self, static_id: str, variables: Union[List[str], str]
    ) -> None:
        """
        ...
        if isinstance(variables, str):
            variables = [variables]
        self._add_output_variables(
            static_id, variables, False, OutputVariableType.FINAL_STATE
        )
    ...
    def _add_output_variables(
        ...
        variables: List[str],
        is_dynamic: bool,
        output_variable_type: OutputVariableType,
    ) -> None:
        _pp.add_output_variables(
            self._handle, element_id, variables, is_dynamic, output_variable_type
        )
