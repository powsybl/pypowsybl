...
class SecurityAnalysisResult:
    ...
    def __init__(self, handle: _pypowsybl.JavaHandle):
        self._handle = handle
        self._pre_contingency_result = _pypowsybl.get_pre_contingency_result(
            self._handle
        )
        post_contingency_results = _pypowsybl.get_post_contingency_results(self._handle)
        operator_strategy_results = _pypowsybl.get_operator_strategy_results(
            self._handle
        )
    ...
    def find_post_contingency_result(
    ...
    def find_operator_strategy_results(
        self, operator_strategy_id: str
    ) -> OperatorStrategyResult:
        """
    ...
    def get_table(self) -> PrettyTable:
        ...
        ) in self._post_contingency_results.items():
            table.add_row(
                [
                    contingency_id,
                    "",
                    post_contingency_result.status.name,
                    "",
                    "",
                    "",
                    "",
        ...
        ) in self._operator_strategy_results.items():
            table.add_row(
                [
                    "",
                    operator_strategy_id,
                    operator_strategy_result.status.name,
                    "",
                    "",
                    "",
                    "",
    ...
    def branch_results(self) -> pd.DataFrame:
        ...
        return create_data_frame_from_series_array(
            _pypowsybl.get_branch_results(self._handle)
        )
    ...
    def bus_results(self) -> pd.DataFrame:
        ...
        """
        return create_data_frame_from_series_array(
            _pypowsybl.get_bus_results(self._handle)
        )
    ...
    def three_windings_transformer_results(self) -> pd.DataFrame:
        ...
        """
        return create_data_frame_from_series_array(
            _pypowsybl.get_three_windings_transformer_results(self._handle)
        )
