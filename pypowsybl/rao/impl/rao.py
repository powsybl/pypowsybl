...
class Rao:
    ...
    def set_crac_file_source(
        self, network: Network, crac_file: Union[str, PathLike]
    ) -> None:
    ...
    def set_crac_buffer_source(self, network: Network, crac_source: io.BytesIO) -> None:
        _pypowsybl.set_crac_source(
            network._handle, self._handle, crac_source.getbuffer()
        )

    def set_glsk_file_source(
        self, network: Network, glsk_file: Union[str, PathLike]
    ) -> None:
    ...
    def set_glsk_buffer_source(self, network: Network, glsk_source: io.BytesIO) -> None:
        _pypowsybl.set_glsk_source(
            network._handle, self._handle, glsk_source.getbuffer()
        )

    def run(
        self, network: Network, parameters: Optional[RaoParameters] = None
    ) -> RaoResult:
        """
        ...
        rao_result = _pypowsybl.run_rao(
            network=network._handle,
            rao_context=self._handle,
            parameters=parameters._to_c_parameters(),
        )
        crac_handle = _pypowsybl.get_crac(self._handle)
        return RaoResult(rao_result, crac_handle)

    def run_voltage_monitoring(
        self,
        network: Network,
        ...
        result_handle = _pypowsybl.run_voltage_monitoring(
            network._handle, rao_result._handle_result, self._handle, p, provider_str
        )
        crac_handle = _pypowsybl.get_crac(self._handle)
        return RaoResult(result_handle, crac_handle)

    def run_angle_monitoring(
        self,
        network: Network,
        rao_result: RaoResult,
        load_flow_parameters: Optional[LfParameters] = None,
        ...
        result_handle = _pypowsybl.run_angle_monitoring(
            network._handle, rao_result._handle_result, self._handle, p, provider_str
        )
        crac_handle = _pypowsybl.get_crac(self._handle)
        return RaoResult(result_handle, crac_handle)
