...
from pypowsybl.loadflow import ComponentResult, Parameters
...
class Backend:
    def __init__(
        ...
        self._handle = _pypowsybl.create_grid2op_backend(
            self._network._handle,
    ...
    def __getstate__(self) -> Dict[str, Any]:
        return {
            "xiidm": self._network.save_to_binary_buffer("XIIDM", {}),
    ...
    def __setstate__(self, state: Dict[str, Any]) -> None:
        self._network = Network(
            _pypowsybl.load_network_from_binary_buffers(
                [state["xiidm"].getbuffer()], {}, [], None, False
            )
        )
        self._consider_open_branch_reactive_flow = state[
            "consider_open_branch_reactive_flow"
        ]
        self._check_isolated_and_disconnected_injections = state[
            "check_isolated_and_disconnected_injections"
        ]
        self._buses_per_voltage_level = state["buses_per_voltage_level"]
        self._connect_all_elements_to_first_bus = state[
    ...
    def update_double_value(
        self,
        value_type: Grid2opUpdateDoubleValueType,
        value: np.ndarray,
        changed: np.ndarray,
    ) -> None:
        _pypowsybl.update_grid2op_double_value(self._handle, value_type, value, changed)

    def update_integer_value(
        self,
        value_type: Grid2opUpdateIntegerValueType,
        value: np.ndarray,
        changed: np.ndarray,
    ) -> None:
        _pypowsybl.update_grid2op_integer_value(
            self._handle, value_type, value, changed
        )

    def check_isolated_and_disconnected_injections(self) -> bool:
        return _pypowsybl.check_grid2op_isolated_and_disconnected_injections(
            self._handle
        )

    def run_pf(
        self, dc: bool = False, parameters: Optional[Parameters] = None
        ...
        return [
            ComponentResult(res)
            for res in _pypowsybl.run_grid2op_loadflow(self._handle, dc, p)
        ]
