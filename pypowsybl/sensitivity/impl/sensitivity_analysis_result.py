...
class SensitivityAnalysisResult:
    ...
    def get_sensitivity_matrix(
        ...
        """
        matrix = _pypowsybl.get_sensitivity_matrix(
            self.result_context_ptr,
            matrix_id,
            self.clean_contingency_id(contingency_id),
        )
        if matrix is None:
            return None
        ...
        df = pd.DataFrame(
            data=data,
            columns=self.functions_ids[matrix_id],
    ...
    def get_reference_matrix(
        ...
        reference_column_id: str = DEFAULT_REFERENCE_COLUMN_ID,
    ) -> Optional[pd.DataFrame]:
        """
        ...
        """
        matrix = _pypowsybl.get_reference_matrix(
            self.result_context_ptr,
            matrix_id,
            self.clean_contingency_id(contingency_id),
        )
