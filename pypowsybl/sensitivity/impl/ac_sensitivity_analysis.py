...
class AcSensitivityAnalysis(SensitivityAnalysis):
    ...
    def set_bus_voltage_factor_matrix(
        self, bus_ids: List[str], target_voltage_ids: List[str]
    ) -> None:
        """
        .. deprecated:: 1.1.0
        ...
        """
        warnings.warn(
            "set_bus_voltage_factor_matrix is deprecated, use add_bus_voltage_factor_matrix instead",
            DeprecationWarning,
        )
        self.add_bus_voltage_factor_matrix(bus_ids, target_voltage_ids)

    def add_bus_voltage_factor_matrix(
        self,
        bus_ids: List[str],
        target_voltage_ids: List[str],
    ...
    def run(
        ...
        sensitivity_parameters = (
            ...
            if isinstance(parameters, LfParameters)
            else parameters
        )
        p: _pypowsybl.SensitivityAnalysisParameters = (
            sensitivity_parameters._to_c_parameters()
            if sensitivity_parameters is not None
            else Parameters()._to_c_parameters()
        )  # pylint: disable=W0212
        return AcSensitivityAnalysisResult(
            _pypowsybl.run_sensitivity_analysis(
                self._handle,
                network._handle,
                False,
                p,
            ...
            functions_ids=self.functions_ids,
