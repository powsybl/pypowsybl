...
def create_extensions(n: Network, slack_weight_by_gen_id: Dict[str, float]) -> None:
    if len(slack_weight_by_gen_id) > 0:
        highest_weight_gen_id = max(
            slack_weight_by_gen_id, key=lambda k: slack_weight_by_gen_id[k]
        )
...
def generate_injection_id(df: pd.DataFrame, prefix: str) -> pd.Series:
    return df.apply(
        lambda row: build_injection_id(prefix, row["bus"], row.name), axis=1
    )
...
def generate_transformer_id(df: pd.DataFrame, index_offset: int) -> pd.Series:
    return df.apply(
        lambda row: build_transformer_id(row, row.name, index_offset), axis=1
    )
...
def create_transformers(n: Network, n_pdp: pandapowerNet) -> None:
    if len(n_pdp.trafo) > 0:
        bus = n_pdp.bus[["vn_kv"]]
        trafo_and_bus = n_pdp.trafo.merge(
            bus.rename(columns=lambda x: x + "_lv_bus"),
            left_on="lv_bus",
            right_index=True,
            how="left",
        )
        ...
        n_tap = np.where(
            ...
            1.0
            + (trafo_and_bus["tap_pos"] - trafo_and_bus["tap_neutral"])
            * trafo_and_bus["tap_step_percent"]
            / 100.0,
            1.0,
        )
        rated_u1 = np.where(
            trafo_and_bus["tap_side"] == "hv",
            trafo_and_bus["vn_hv_kv"] * n_tap,
            trafo_and_bus["vn_hv_kv"],
        )
        rated_u2 = np.where(
            trafo_and_bus["tap_side"] == "lv",
            trafo_and_bus["vn_lv_kv"] * n_tap,
            trafo_and_bus["vn_lv_kv"],
        )
        ...
        n.create_2_windings_transformers(
            id=trafo_id,
            name=name,
            voltage_level1_id=vl1_id,
            connectable_bus1_id=connectable_bus1_id,
            bus1_id=bus1_id,
            voltage_level2_id=vl2_id,
            connectable_bus2_id=connectable_bus2_id,
            bus2_id=bus2_id,
            rated_u1=rated_u1,
            rated_u2=rated_u2,
            r=r,
...
def create_lines(n: Network, n_pdp: pandapowerNet) -> None:
    if len(n_pdp.line) > 0:
        ...
        r = (
            n_pdp.line["length_km"]
            * n_pdp.line["r_ohm_per_km"]
            / n_pdp.line["parallel"]
        )
        x = (
            n_pdp.line["length_km"]
            * n_pdp.line["x_ohm_per_km"]
            / n_pdp.line["parallel"]
        )
        g = (
            n_pdp.line["length_km"]
            * n_pdp.line["g_us_per_km"]
            * 1e-6
            * n_pdp.line["parallel"]
            / 2
        )
...
def create_shunts(n: Network, n_pdp: pandapowerNet) -> None:
    if len(n_pdp.shunt) > 0:
        ...
        g_per_section = (
            n_pdp.shunt["p_mw"] / (n_pdp.shunt["vn_kv"] ** 2) * -1.0
        ).tolist()
        b_per_section = (
            n_pdp.shunt["q_mvar"] / (n_pdp.shunt["vn_kv"] ** 2) * -1.0
        ).tolist()
...
def _create_generators(
    ...
    bus: DataFrame,
    slack_weight_by_gen_id: Dict[str, float],
    generator_type: PandaPowerGeneratorType,
) -> None:
    if len(gen) > 0:
        gen_and_bus: DataFrame = gen.merge(
            bus, left_on="bus", right_index=True, how="left", suffixes=("", "_x")
        )
        gen_id = generate_injection_id(
            gen_and_bus,
            (
                "gen"
        ...
        voltage_regulator_on = [
            generator_type != PandaPowerGeneratorType.STATIC_GENERATOR
        ] * len(gen_and_bus)
        ...
        min_q = (
            gen_and_bus["min_q_mvar"] if "min_q_mvar" in gen_and_bus.columns else None
        )
        max_q = (
            gen_and_bus["max_q_mvar"] if "max_q_mvar" in gen_and_bus.columns else None
        )
...
def create_generator_target_p(
    gen_and_bus: DataFrame, generator_type: PandaPowerGeneratorType
) -> pd.Series:
    return (
        pd.Series(
            [MIN_TARGET_P_TO_NOT_BE_DISCADED_FROM_ACTIVE_POWER_CONTROL]
            * len(gen_and_bus)
        )
        if generator_type == PandaPowerGeneratorType.EXT_GRID
        else gen_and_bus["p_mw"]
    )  # keep a small value of target_p to avoid be discarded to slack distribution


def create_generator_max_p(
    gen_and_bus: DataFrame, generator_type: PandaPowerGeneratorType
) -> pd.Series:
    return (
        pd.Series([DEFAULT_MAX_P] * len(gen_and_bus))
        if generator_type == PandaPowerGeneratorType.EXT_GRID
        or "max_p_mw" not in gen_and_bus.columns
        else pd.Series(np.nan_to_num(gen_and_bus["max_p_mw"], nan=DEFAULT_MAX_P))
    )


def create_generator_min_p(
    gen_and_bus: DataFrame, generator_type: PandaPowerGeneratorType
) -> pd.Series:
    return (
        pd.Series([DEFAULT_MIN_P] * len(gen_and_bus))
        if generator_type == PandaPowerGeneratorType.EXT_GRID
        or "min_p_mw" not in gen_and_bus.columns
        else pd.Series(np.nan_to_num(gen_and_bus["min_p_mw"], nan=DEFAULT_MIN_P))
    )


def create_generator_target_q(
    gen_and_bus: DataFrame, generator_type: PandaPowerGeneratorType
) -> pd.Series:
    return (
        gen_and_bus["q_mvar"]
        if generator_type == PandaPowerGeneratorType.STATIC_GENERATOR
        and "q_mvar" in gen_and_bus.columns
        else pd.Series([0.0] * len(gen_and_bus))
    )


def create_generator_target_v(
    gen_and_bus: DataFrame, generator_type: PandaPowerGeneratorType
) -> pd.Series:
    return (
        gen_and_bus["vm_pu"] * gen_and_bus["vn_kv"]
        if generator_type != PandaPowerGeneratorType.STATIC_GENERATOR
        else pd.Series([1.0] * len(gen_and_bus))
    )


def fill_generator_slack_weight(
    gen_and_bus: DataFrame,
    generator_type: PandaPowerGeneratorType,
    slack_weight_by_gen_id: Dict[str, float],
) -> None:
    if generator_type != PandaPowerGeneratorType.STATIC_GENERATOR:
        for index, row in gen_and_bus.iterrows():
            index = typing.cast(
                int, index
            )  # safe cas from hashtable to int needed by mypy
            weight = (
                row["slack_weight"]
                if generator_type == PandaPowerGeneratorType.EXT_GRID or row["slack"]
                else 0.0
            )
            slack_weight_by_gen_id[build_injection_id("gen", row["bus"], index)] = (
                weight
            )


def create_generators(
    n: Network, n_pdp: pandapowerNet, slack_weight_by_gen_id: Dict[str, float]
) -> None:
    _create_generators(
        n,
        n_pdp.gen,
        ...
        PandaPowerGeneratorType.GENERATOR,
    )
    _create_generators(
        n,
        n_pdp.ext_grid,
        n_pdp.bus,
        slack_weight_by_gen_id,
        PandaPowerGeneratorType.EXT_GRID,
    )
    _create_generators(
        n,
        n_pdp.sgen,
        n_pdp.bus,
        slack_weight_by_gen_id,
        PandaPowerGeneratorType.STATIC_GENERATOR,
    )
...
def create_buses(n: Network, n_pdp: pandapowerNet) -> None:
    if len(n_pdp.bus) > 0:
        ...
        low_voltage_limit = (
            n_pdp.bus["min_vm_pu"] * nominal_v
            if "min_vm_pu" in n_pdp.bus.columns
            else None
        )
        high_voltage_limit = (
            n_pdp.bus["max_vm_pu"] * nominal_v
            if "max_vm_pu" in n_pdp.bus.columns
            else None
        )
