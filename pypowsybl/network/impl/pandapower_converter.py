...
def create_transformers(n, n_pdp):
    if len(n_pdp.trafo) > 0:
        ...
        vl1_id = build_voltage_level_id(trafo_and_bus['lv_bus'].astype(str))
        vl2_id = build_voltage_level_id(trafo_and_bus['hv_bus'].astype(str))
        bus1_id = build_bus_id(trafo_and_bus['lv_bus'].astype(str))
        bus2_id = build_bus_id(trafo_and_bus['hv_bus'].astype(str))
        rated_u1 = trafo_and_bus['vn_lv_kv']
        rated_u2 = trafo_and_bus['vn_hv_kv']
        r_pu = trafo_and_bus['vkr_percent'] / 100
        z_pu = trafo_and_bus['vk_percent'] / 100
        x_pu = np.sqrt(z_pu ** 2 - r_pu ** 2)
        y_pu = trafo_and_bus['i0_percent'] / 100
        g_pu = trafo_and_bus['pfe_kw'] / (trafo_and_bus['sn_mva'] * 1000)
        b_pu = np.sqrt(y_pu ** 2 - g_pu ** 2)

        vn2 = trafo_and_bus['vn_kv_lv_bus']
        to_eu = vn2 ** 2 / trafo_and_bus['sn_mva']
        r = r_pu * to_eu
        x = x_pu * to_eu
        g = g_pu / to_eu
        b = b_pu / to_eu
