...
class NodeBreakerTopology:
    ...
    def __init__(self, network_handle: _pp.JavaHandle, voltage_level_id: str):
        self._internal_connections = create_data_frame_from_series_array(
            _pp.get_node_breaker_view_internal_connections(
                network_handle, voltage_level_id
            )
        )
    ...
    def create_graph(self) -> _nx.Graph:
        ...
        for index, row in self.nodes.iterrows():
            graph.add_node(
                index,
                connectable_id=row["connectable_id"],
                connectable_type=row["connectable_type"],
            )
        ...
        graph.add_edges_from(
            self._internal_connections[["node1", "node2"]].values.tolist()
        )
        return graph
