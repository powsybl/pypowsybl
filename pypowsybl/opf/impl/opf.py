...
class OptimalPowerFlow:
    ...
    def run(self):
        ...
        shunts = self._network.get_shunt_compensators()
        lines = self._network.get_lines()
        transfos = self._network.get_2_windings_transformers(all_attributes=True)
        branches = self._network.get_branches()

        slack_terminal = self._network.get_extensions('slackTerminal')
