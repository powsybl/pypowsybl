...
Given a JSON OpenRAO CRAC file, you can import the CRAC data in pypowsybl using ``Crac.load``:

.. code-block:: python

    crac = Crac.load(network, "path/to/crac.json")
...
to the ``Crac.load`` method. A JSON example is provided below.
...
.. code-block:: python
    ...
    crac = Crac.load(network, "path/to/crac.json", creation_parameters="path/to/parameters.json")
