...
Other kind of distribution factors
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Pypowsybl allows to compute more that Power Transfer Distribution Factor. In addition, you can:

- put id of a phase shifter present in the network to compute the sensitivity of a phase shifting on a line, feature also called Phase Shift Distribution Factor (PSDF) ;
- put the id of an HVDC line if you want to see the effect of an increase of the active power set point on a other line (better known as DCDF). Note that in that case, the HVDC line must be explicitly described in the network through `HvdcLine` object. If the HVDC line is modeled with two injections because the HVDC line is not explicitly modeled (as in network coming from UCTE format), you have to put both injection ids and make the difference between the sensitivity results.
