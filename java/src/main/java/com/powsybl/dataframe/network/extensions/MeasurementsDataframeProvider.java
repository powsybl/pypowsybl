...
public class MeasurementsDataframeProvider extends AbstractSingleDataframeNetworkExtension implements NetworkExtensionDataframeProvider {
    ...
    private Stream<MeasurementInformation> itemsStream(Network network) {
        return network.getConnectableStream()
                .filter(Objects::nonNull)
                .map(connectable -> Pair.of(connectable.getId(), (Measurements) connectable.getExtension(Measurements.class)))
