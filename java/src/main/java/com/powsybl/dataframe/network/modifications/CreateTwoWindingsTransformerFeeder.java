 ...
 */
public class CreateTwoWindingsTransformerFeeder implements NetworkModification {
    @Override
    public List<List<SeriesMetadata>> getMetadata(DataframeElementType elementType) {
        return Collections.singletonList(FeederBaysTwtSeries.getSeriesMetadata());
    }
    ...
    public void applyModification(Network network, List<UpdatingDataframe> dataframe, boolean throwException, ReporterModel reporter) {
        ...
        CreateBranchFeederBaysBuilder builder = fbTwtSeries.createBuilder(network, dataframe.get(0));
